...
// HandleDBDeletion handles deletion of the resources related to the database Cluster
// Returns true if the deletion is complete.
func HandleDBDeletion(
	ctx context.Context,
	c client.Client,
	database *everestv1alpha1.DatabaseCluster,
	...
	if controllerutil.ContainsFinalizer(database, DBDeleteFinalizer) {
		...
		controllerutil.RemoveFinalizer(database, DBDeleteFinalizer)
		return true, c.Update(ctx, database)
	}
	return true, nil
}

// Checks if the created for the DB resources are deleted.
// Returns true if the deletion is complete.
func checkDatabaseDeletion(
	ctx context.Context,
	c client.Client,
	dbName, dbNs string,
) (bool, error) {
	...
	restoresList, err := ListDatabaseClusterRestores(ctx, c, dbName, dbNs)
	if err != nil {
		return false, err
	}
	if len(backupList.Items) == 0 && len(restoresList.Items) == 0 {
		return true, nil
	}
