...
func main() {
	...
	if err = (&scheduledbackup.DatabaseClusterPSMDBBackupReconciler{
		Client: mgr.GetClient(),
		Scheme: mgr.GetScheme(),
	}).SetupWithManager(mgr); err != nil {
		setupLog.Error(err, "unable to create controller", "controller", "DatabaseClusterPSMSBBackup")
		os.Exit(1)
	}
	if err = (&scheduledbackup.DatabaseClusterPGBackupReconciler{
		Client: mgr.GetClient(),
		Scheme: mgr.GetScheme(),
	}).SetupWithManager(mgr); err != nil {
		setupLog.Error(err, "unable to create controller", "controller", "DatabaseClusterPGBackup")
		os.Exit(1)
	}
